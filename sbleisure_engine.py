...
def validasi_pickup(lokasi):
    ...
    """
    if not lokasi:
        return False, 0.0[cite: 18]

    lokasi_clean = str(lokasi).strip().lower()[cite: 18]
    kawasan_dibenarkan = [
    ...
    sah = any(k in lokasi_clean for k in kawasan_dibenarkan)[cite: 18]
    harga_asas = dapatkan_harga_asas_destinasi(lokasi_clean) if sah else 0.0[cite: 18]
    return sah, harga_asas[cite: 18]

def dapatkan_harga_asas_destinasi(destinasi):
    """Menentukan harga asas (50 km pertama) berdasarkan zon mukim destinasi"""
    dest = str(destinasi).strip().lower()[cite: 18]
    
    zon_700 = ["ampang", "cheras", "hulu langat", "batu", "setapak", "ulu kelang", "kuala lumpur", "pusat bandar", "bukit bintang", "chow kit", "brickfields", "bangsar", "seputeh", "kepong", "segambut", "sentul", "jalan ipoh", "mont kiara", "sri hartamas", "batu caves", "wangsa maju", "danau kota", "taman melati", "semarak", "ampang hilir", "kampung pandan", "desa pandan", "maluri"][cite: 18]
    if any(z in dest for z in zon_700):
        return 700.00[cite: 18]

    zon_800 = ["klia", "cyberjaya", "putrajaya", "airport", "damansara", "petaling", "kajang", "semenyih", "rawang", "dengkil", "sepang", "labu"][cite: 18]
    if any(z in dest for z in zon_800):
        return 800.00[cite: 18]

    zon_900 = ["bukit raja", "sungai buloh", "sg buloh", "beranang", "kapar", "klang"][cite: 18]
    if any(z in dest for z in zon_900):
        return 900.00[cite: 18]

    zon_1000 = ["bandar", "jugra", "kelanang", "morib", "tanjong duabelas", "telok panglima garang", "ampang pecah", "batang kali", "buloh telor", "kalumpang", "kerling", "kuala kalumpang", "peretak", "rasa", "serendah", "sungai gumut", "sungai tinggi", "ulu bernam", "ulu yam"][cite: 18]
    if any(z in dest for z in zon_1000):
        return 1000.00[cite: 18]

    zon_1200 = ["api-api", "batang berjuntai", "bestari jaya", "ijok", "jeram", "kuala selangor", "pasangan", "tanjong karang", "ujong pematang", "ulu tinggi", "bagan nakhoda omar", "panchang bedena", "pasiran panjang", "sabak", "sungai panjang", "ampangan", "lenggeng", "pantai", "rantau", "rasah", "seremban", "setul", "jimah", "linggi", "pasir panjang", "port dickson", "si rusa", "tapah"][cite: 18]
    if any(z in dest for z in zon_1200):
        return 1200.00[cite: 18]

    zon_1400 = ["batu kikir", "chembong", "gadong", "kota", "kundor", "legong hilir", "legong hulu", "mambau", "nerasau", "pedas", "pilin", "seberang", "titian bintangor", "batu hampar", "gadong hilir", "rembau bandar", "ampang tinggi", "johol", "juasseh", "kepas", "kuala pilah", "langkap", "seri menanti", "ulu jempol", "terachi", "parit tinggi", "glami lemi", "hulu klawang", "klawang", "kuala klawang", "pertang", "peradong", "kenaboi", "triang hilir", "ulu triang"][cite: 18]
    if any(z in dest for z in zon_1400):
        return 1400.00[cite: 18]

    zon_1600 = ["jelai", "serting ilir", "serting hulu", "palong", "ayer kuning", "gemencheh", "gemas", "kepis", "ladang", "tampin tengah", "tampin", "ipoh", "taiping", "melaka", "kuantan", "johor bahru"][cite: 18]
    if any(z in dest for z in zon_1600):
        return 1600.00[cite: 18]
...
def semak_jadual_tetap(pickup, dest):
    ...
    p = pickup.strip().lower()[cite: 18]
    d = dest.strip().lower()[cite: 18]

    for (asal, tuju), harga in JADUAL_HARGA_BAS_TETAP.items():[cite: 18]
        if (asal in p and tuju in d) or (tuju in p and asal in d):[cite: 18]
            return float(harga)[cite: 18]
    return None[cite: 18]
...
def tentukan_zon_wilayah(destinasi):
    ...
    dest = str(destinasi).lower()[cite: 18]
    
    zon_utara = ["rawang", "bukit beruntung", "kalumpang", "tanjung malim", "tapah", "ipoh", "taiping", "teluk intan", "parit buntar", "gerik", "kuala kangsar", "sungai buloh", "serendah", "batang kali", "rasa", "hulu bernam", "butterworth", "georgetown", "george town", "batu ferringhi", "bayan lepas", "pulau pinang", "penang", "alor setar", "kangar"][cite: 18]
    zon_selatan = ["kajang", "semenyih", "seremban", "port dickson", "nilai", "sepang", "dengkil", "bangi", "mantin", "rembau", "tampin", "melaka", "muar", "batu pahat", "johor bahru", "kuala klawang", "klawang"][cite: 18]
    zon_timur = ["janda baik", "bukit tinggi", "genting", "bentong", "raub", "temerloh", "kuantan", "cameron highlands", "kuala lipis"][cite: 18]
    
    if any(k in dest for k in zon_utara):[cite: 18]
        return "utara"[cite: 18]
    elif any(k in dest for k in zon_selatan):[cite: 18]
        return "selatan"[cite: 18]
    elif any(k in dest for k in zon_timur):[cite: 18]
        return "timur"[cite: 18]
    else:
        return "standard"[cite: 18]

def dapatkan_jarak_km(lokasi_asal, lokasi_tuju):
    """Mendapatkan jarak pemanduan sebenar (km) menggunakan OpenStreetMap / OSRM"""
    try:
        headers = {'User-Agent': 'SBLTransportApp/1.0'}[cite: 18]
        url_asal = f"https://nominatim.openstreetmap.org/search?q={lokasi_asal},Malaysia&format=json&limit=1"[cite: 18]
        url_tuju = f"https://nominatim.openstreetmap.org/search?q={lokasi_tuju},Malaysia&format=json&limit=1"[cite: 18]
        
        res_asal = requests.get(url_asal, headers=headers, timeout=4).json()[cite: 18]
        res_tuju = requests.get(url_tuju, headers=headers, timeout=4).json()[cite: 18]
        
        if not res_asal or not res_tuju:[cite: 18]
            return None[cite: 18]
            
        lon1, lat1 = res_asal[0]['lon'], res_asal[0]['lat'][cite: 18]
        lon2, lat2 = res_tuju[0]['lon'], res_tuju[0]['lat'][cite: 18]
        
        osrm_url = f"http://router.project-osrm.org/route/v1/driving/{lon1},{lat1};{lon2},{lat2}?overview=false"[cite: 18]
        osrm_res = requests.get(osrm_url, timeout=4).json()[cite: 18]
        
        if osrm_res.get('routes'):[cite: 18]
            jarak_meter = osrm_res['routes'][0]['distance'][cite: 18]
            return round(jarak_meter / 1000.0, 1)[cite: 18]
    except Exception:[cite: 18]
        return None[cite: 18]
    return None[cite: 18]

def bundar_ke_puluhan_atas(nilai):
    return math.ceil(nilai / 10.0) * 10[cite: 18]
...
def kira_harga_bas(lokasi_pickup, destinasi, jarak_km=None, tier_jalan="normal"):
    ...
    """
    pickup = str(lokasi_pickup).strip().lower()[cite: 18]
    dest = str(destinasi).strip().lower()[cite: 18]
    
    # 1. Semak Jadual Tetap
    harga_tetap = semak_jadual_tetap(pickup, dest)[cite: 18]
    if harga_tetap is not None:[cite: 18]
        jumlah_harga = harga_tetap
    else:
        # 2. Dapatkan Harga Asas daripada Zon Destinasi (50 km pertama)
        harga_asas = dapatkan_harga_asas_destinasi(dest)[cite: 18]
            
        # 3. Dapatkan Jarak Sebenar jika tidak dibekalkan
        if jarak_km is None or jarak_km <= 0:[cite: 18]
            jarak_dikesan = dapatkan_jarak_km(pickup, dest)[cite: 18]
            jarak_km = jarak_dikesan if jarak_dikesan else 80.0[cite: 18]
            
        # 4. Tentukan Kadar Lebihan KM Mengikut Zon Wilayah (51 km ke atas)
        zon = tentukan_zon_wilayah(dest)[cite: 18]
        if zon == "utara":[cite: 18]
            kadar_km = 3.50[cite: 18]
        elif zon == "selatan":[cite: 18]
            kadar_km = 4.00[cite: 18]
        elif zon == "timur":[cite: 18]
            kadar_km = 6.50[cite: 18]
        else:
            kadar_km = 3.50[cite: 18]
            
        caj_jarak = (jarak_km - 50) * kadar_km if jarak_km > 50 else 0[cite: 18]
        jumlah_harga = harga_asas + caj_jarak[cite: 18]
    ...
    if tier_jalan in ["berbukit", "sempit", "pedalaman"]:[cite: 18]
        jumlah_harga *= 1.15[cite: 18]
        
    return float(bundar_ke_puluhan_atas(jumlah_harga))[cite: 18]
...
def kira_harga_trip(lokasi_pickup, destinasi, jenis_trip="one_way", return_hari_sama=True, jarak_km=None, tier_jalan="normal"):
    ...
    """
    harga_one_way = kira_harga_bas(lokasi_pickup, destinasi, jarak_km=jarak_km, tier_jalan=tier_jalan)[cite: 18]
    
    trip = str(jenis_trip).strip().lower()[cite: 18]
    if trip in ["two_way", "two way", "pergi balik", "return"]:[cite: 18]
        if return_hari_sama:[cite: 18]
            jumlah_two_way = harga_one_way * 1.5[cite: 18]
        else:
            jumlah_two_way = harga_one_way * 2.0[cite: 18]
        return float(bundar_ke_puluhan_atas(jumlah_two_way))[cite: 18]
        
    return float(harga_one_way)[cite: 18]

def semak_tarikh_booking(tarikh_str):
    """Menyemak sama ada tarikh tempahan melepasi sekatan tempahan tergesa-gesa (>= 8 hari)"""
    try:
        tarikh_bersih = tarikh_str.strip()[cite: 18]
        if '-' in tarikh_bersih and len(tarikh_bersih.split('-')[0]) == 4:[cite: 18]
            tarikh_booking = datetime.strptime(tarikh_bersih, "%Y-%m-%d").date()[cite: 18]
        else:
            tarikh_booking = datetime.strptime(tarikh_bersih, "%d-%m-%Y").date()[cite: 18]
        
        tarikh_semasa = datetime.now().date()[cite: 18]
        selisih_hari = (tarikh_booking - tarikh_semasa).days[cite: 18]
        
        if selisih_hari < 0:[cite: 18]
            return "tidak_sah", "Tarikh yang dipilih sudah lepas."[cite: 18]
        elif selisih_hari <= 7:[cite: 18]
            return "urgent", "Maaf, untuk tempahan dalam masa 7 hari atau kurang (urgent booking), sistem tidak boleh terima. Sila berhubung terus dengan sales team kami di pautan ini: https://wa.link/nrmesv"[cite: 18]
        else:
            return "boleh", "Tarikh disahkan lulus untuk tempahan online."[cite: 18]
    except ValueError:[cite: 18]
        return "ralat", "Format tarikh tidak sah. Sila guna format YYYY-MM-DD atau DD-MM-YYYY."[cite: 18]
...
def dapatkan_harga_automatik(pickup, destinasi, jenis_trip="one_way", return_hari_sama=True, tier_jalan="normal"):
    ...
    return kira_harga_trip(pickup, destinasi, jenis_trip=jenis_trip, return_hari_sama=return_hari_sama, jarak_km=None, tier_jalan=tier_jalan)[cite: 18]